...
export default function SystemReferencesPage() {
  const router = useRouter();
  const [references, setReferences] = useState<SystemReference[]>([]);
  const [loading, setLoading] = useState(true);
  const [deleteTarget, setDeleteTarget] = useState<SystemReference | null>(null);

  useEffect(() => {
    fetch("/api/settings/system-references")
      .then((r) => r.json())
      .then((data) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      await fetch(`/api/settings/system-references/${deleteTarget.id}`, { method: "DELETE" });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">参考原稿</h2>
          <p className="text-sm text-gray-500 mt-1">
            応募実績のある求人原稿を登録しておくと、全組織の原稿生成・改善時に AI が自動で参考にします。
          </p>
        </div>
        <Link href="/settings/system-references/new">
          <Button size="sm">
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            router.push(`/settings/system-references/${ref.id}/edit`);
                          }}
                        >
